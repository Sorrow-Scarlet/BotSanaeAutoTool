...
namespace 早苗日刷工具
{
    ...
    public class GetConfig
    {
        public string[] OnInitializing()//Config初始化，分为存在与不存在Config
        {
            ...
            {
                //默认的Config板式
                string[] originalConfig = { @"{", "\t\"GroupName\": \"\",","\t\"DailyContents\": \"签到\",", "\t\"RegularContents\": \"喂食\",", "\t\"RC_TimeSection\": [\"07:30\", \"12:05\", \"17:05\", \"23:05\"],", "\t\"Contents\": [\"钓鱼\", \"打工\", \"打工\", \"打工 2\"]", "}" };
                File.Create(@".//Config,json");//因为没有检测到Config.json，创建一个新的Config.json
                File.WriteAllLines(pathConfig, originalConfig);//写入默认的Config板式
                string[] ReadConfig = File.ReadAllLines(pathConfig);
                return ReadConfig;
            }
